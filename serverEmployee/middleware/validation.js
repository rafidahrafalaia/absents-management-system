...
const jwt_decode = require('jwt-decode');
  ...
  async function authenticateToken(req, res, next) {
    ...
    console.log(token,'aslaksl')
    if(token){
    jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, (err, user)=>{
      console.log(err,user,'skkskak')
  ...
  async function isEmployee(req, res, next) {
    const user = req.user;
    if(!user.role.length || user?.role[0].name !== "employee") res.status(400).json({ message:"user not allowed for this" });
    else next();
  }
